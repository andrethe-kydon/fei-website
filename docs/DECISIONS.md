...
## Blocking: eleven PDFs naming the old enquiry address

The site moved to `enquiry@futureedgeinstitute.com` on 1 September 2026. Eleven
rendered PDFs did not, and cannot be driven from the settings field: the address
is drawn into the document.

| Item | Decided by | Note |
| --- | --- | --- |
| Nine course and workshop brochures, plus the programme directory | David | `static/assets/brochures/`. Each names `sales@kydongrp.com` three times, the directory twice. They already needed reissuing for the retired orange palette and the old text wordmark, so the address joins that queue rather than creating a new one. Until then the site links to PDFs giving an address the site itself does not use. |
| The two OPC schedule PDFs | David, with Kydon for the workbook | Version 1, 1 September 2026, built by hand from the approved workbook. The overview names the old address twice and the detailed schedule four times. The page now offers the day by day schedule by email as well as by download, so pulling the detailed PDF is an option if reissuing it is not. |
