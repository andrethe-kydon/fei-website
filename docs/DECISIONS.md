...
| Module titles, page versus PDF | Modules and schedule | David | Four of the eight differ between the document and the approved workbook the PDFs were built from. Module 1 is "Gear Up, Essential Career Skills" on the page and "Gear Up: Mindset and Business Foundation" in the PDF; 3, 5 and 8 differ similarly. The page shows one set, the download the other, and a reader opening both will notice. Not changed unasked: the workbook titles are the ones SP approved, so the page is probably the side that should move. |
| "Kydon led" as a delivered by value | Schedule module table | David | The module `deliveredBy` field reads "Kydon led", which is fine in the module list as "43.5 hours, Kydon led" but reads oddly under a Delivered by column, where the other value is "Singapore Polytechnic". The PDF says "Kydon Group". One word in the Studio; not changed unasked. |
